...
这个仓库也包含一个 `desktop-widget-v2` 方向的桌宠版本，额外带有：

- 专用 OpenClaw 聊天桥（`bridge-server.js`）
- 可复用参考音色的英文语音回复（`voice-reference.wav`）
- 独立控制窗，支持文字 + 麦克风输入（`src/controls.html`）
- 通过自然语言直接触发任务执行和 coding helper 脚本
- 一个可复用的技能定义：`desktop-widget-v2-skill/SKILL.md`
